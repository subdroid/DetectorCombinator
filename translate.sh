...
job_names=("t_m1" "t_m2" "t_m3" "t_m4" "t_m5")
error_files=("m1.log" "m2.log" "m3.log" "m4.log" "m5.log")
out_files=("m1.out" "m2.out" "m3.out" "m4.out" "m5.out")
...
for id in 0 1 2 3 4; do
    ...
    if [ "$id" -eq 0 ]; then
        ...
        sbatch --job-name $job_name --output $out_file --error $error_file -p gpu-ms --mem=64G --constraint="gpuram48G" slurm_runner.sh $id
    elif [ "$id" -eq 2 ]; then
        sbatch --job-name $job_name --output $out_file --error $error_file -p gpu-troja --mem=64G --constraint="gpuram48G|gpuram40G" slurm_runner.sh $id
    elif [ "$id" -eq 3 ]; then
